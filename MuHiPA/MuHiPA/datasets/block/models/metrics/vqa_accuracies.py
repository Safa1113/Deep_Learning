...
class VQAAccuracies(nn.Module):
    ...
    def forward(self, cri_out, net_out, batch):
        ...
        #net_out = net_out['answer_ids'].cpu() 
        #batch['answer_id'] = batch['answer_id'].cpu() 
    ...
    def compute_oe_accuracy(self):
        ...
        if 'test' not in self.dataset.split:
            call_to_prog = 'python -m  EMuRelPA.datasets.block.models.metrics.compute_oe_accuracy '\
    ...
    def compute_tdiuc_metrics(self):
        ...
        self.pred_aids = torch.tensor(self.pred_aids, device = 'cpu')
        print("33333333333333", type(self.pred_aids))
        print("555555555555", type(self.gt_aids[0]))
