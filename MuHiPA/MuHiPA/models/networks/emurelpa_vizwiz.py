...
class DyIntraModalityUpdate(nn.Module):
    ...
    def forward(self, v, t):
        ...
        t4v_gate = self.sigmoid(self.t4v_gate_lin(self.drop(self.relu(t)))) #[batch, 1, feat_size]
        ...
        new_vq = (1 + t4v_gate) * v_q
        new_vk = (1 + t4v_gate) * v_k
        ...
        for i in range(self.num_head):
            ...
            # calculate attention
            v2v = (vq_slice @ vk_slice.transpose(1,2)) / ((self.output_size // self.num_head) ** 0.5)
...
class EMuRelPA_VizWiz(nn.Module):
    ...
    def __init__(self,
            ...
            aid_to_ans=[],
            ans_to_aid={}):
        super(EMuRelPA_VizWiz, self).__init__()
...
#        self.mmlast = nn.Linear(4096, 4096)
        ...
        if self.shared:
            self.cell = EMuRelPAReasoning(**cell)
        else:
            self.cells = nn.ModuleList([EMuRelPAReasoning(**cell) for i in range(self.n_step)])
...
#            params += [p.numel() for p in self.q_att_linear1.parameters() if p.requires_grad]
    ...
    def forward(self, batch):
        ...
        # print("0000000000000000000000000", v.shape)
